...
All configuration lives in `.env` (gitignored). Copy `.env.example` and fill the placeholders.
The host-running gateway uses `127.0.0.1:<external-port>` URLs because it talks to the
Docker containers via their host-published ports.

| Variable | Example value | Purpose |
|---|---|---|
| `GATEWAY_PORT` | `8443` | HTTPS port for the gateway |
| `NODE_ENV` | `development` | `production` enforces mTLS strictly |
| `JWT_SECRET` | random 256-bit hex | Reserved for future JWT use |
| `MONGO_ROOT_USER` | `admin` | MongoDB admin user |
| `MONGO_ROOT_PASSWORD` | strong random | MongoDB admin password |
| `MONGODB_URI` | `mongodb://admin:…@127.0.0.1:27020/verichain?authSource=admin&directConnection=true` | Gateway → MongoDB |
| `REDIS_PASSWORD` | strong random | Redis AUTH |
| `REDIS_URL` | `redis://:…@127.0.0.1:6380` | Gateway → Redis |
| `BLOCKCHAIN_RPC` | `http://127.0.0.1:8545` | Hardhat node JSON-RPC |
| `ACCESS_POLICY_ADDRESS` | `0x9fE4…6e0` | Deployed AccessPolicy address (auto-filled by deploy script) |
| `AUDIT_LEDGER_ADDRESS` | `0xCf7E…0Fc9` | Deployed AuditLedger address |
| `GATEWAY_PRIVATE_KEY` | `0xac0974…` | Hardhat account #0 — DEV ONLY |
| `AI_HMAC_SECRET` | random 256-bit hex | Shared secret for gateway↔AI HMAC |
| `AI_ENGINE_URL` | `http://127.0.0.1:5001` | Gateway → AI Engine |
| `ADMIN_API_KEY` | random 192-bit hex | Header key for `/api/admin/*` |
| `REACT_APP_ADMIN_API_KEY` | same value | Embedded in dashboard build |
| `TRUST_DASHBOARD_ORIGIN` | `http://localhost:3005` | CORS allowlist |
| `DESKTOP_AGENT_ORIGIN` | `https://localhost:8443` | mTLS-pinned gateway URL the agent talks to |
| `REACT_APP_GATEWAY_URL` | `https://localhost:8443` | Dashboard → gateway |
| `GATEWAY_CERT_PIN` | `sha256//…` | Pinned cert fingerprint (auto-regenerated by start-all.ps1) |
